...
int Plot2dposition(
    ...
    if (eventNumber < 0 || eventNumber >= tree->GetEntries()) {
        cout << "Invalid event number. The tree contains " << tree->GetEntries() << " events." << endl;
        f->Close();
        return 1;
    }
    
    stringstream eventSelectStr;
    eventSelectStr << "Entry$==" << eventNumber;
    ...
    cout << "Plotting event " << eventNumber << " from tree " << treeName << endl;
    ...
    titleStr << "Photon Distribution - Event " << eventNumber << " (" << xBins << "x" << yBins << " bins)";
    ...
    outFileStr << "2DHistogram_" << xBins << "x" << yBins << "_event" << eventNumber << ".png";
